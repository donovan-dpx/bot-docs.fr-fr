...
Lorsque le modèle renvoie un résultat, il indique quel service est le mieux à même de gérer l’énoncé. Le code de ce bot achemine la demande vers le service correspondant, puis il résume la réponse du service appelé. Selon l’ _ retournée par Dispatch, ce code utilise l’intention retournée pour router vers le modèle LUIS ou service QnA approprié.
...
1. Ouvrez votre émulateur Bot Framework Emulator, puis sélectionnez `Create a new bot configuration`. Un fichier `.bot` vous permet d’utiliser l’ _ dans l’émulateur de bot pour voir le code JSON retourné à partir de LUIS et de QnA Maker.
...
Quand vous combinez l’application LUIS et l’application QnA Maker à l’aide de l’outil Dispatch, vous devez effectuer l’ _ des opérations suivantes :
